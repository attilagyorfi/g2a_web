...
export default function AdminCategories() {
  ...
  const createMutation = trpc.admin.categories.create.useMutation({ onSuccess: () => { refetch(); setShowForm(false); setForm(EMPTY); toast.success("Kategória létrehozva"); } });
  ...
  const deleteManyMutation = trpc.admin.categories.deleteMany.useMutation({
    onSuccess: (n) => { refetch(); toast.success(`${n} kategória törölve`); selection.clear(); },
  });
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<number | null>(null);
  const [form, setForm] = useState<Form>(EMPTY);
  const selection = useBulkSelection<number>((items || []).map(i => i.id));
  const handleBulkDelete = async () => {
    const ok = await confirm({ title: "Tömeges törlés", message: `Biztosan törlöd a kiválasztott ${selection.count} kategóriát?` });
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
        <h1 style={{ color: "#fff", fontFamily: "Geist Mono, monospace", fontSize: "1.5rem", fontWeight: 700 }}>Kategóriák</h1>
      ...
      {showForm && (
        <div style={{ backgroundColor: "#1a1a1a", border: "1px solid rgba(20,184,166,0.3)", borderRadius: "8px", padding: "1.5rem", marginBottom: "1.5rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
            <h2 style={{ color: "#fff", fontFamily: "Geist Mono, monospace", fontSize: "1rem" }}>{editing ? "Szerkesztés" : "Új kategória"}</h2>
          ...
          <form onSubmit={e => { e.preventDefault(); editing ? updateMutation.mutate({ id: editing, data: form }) : createMutation.mutate(form); }} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            <LocalizedField label="Név" field="name" form={form} setForm={setForm} required placeholder="Kategória neve" />
            <div><label style={labelStyle}>Slug</label><input value={form.slug} onChange={e => setForm(p => ({ ...p, slug: e.target.value }))} style={inputStyle} placeholder="url-slug" /></div>
      ...
      <BulkActionBar count={selection.count} itemLabel="kiválasztott kategória" onClear={selection.clear} onDelete={handleBulkDelete} deleting={deleteManyMutation.isPending} />
      <div style={{ backgroundColor: "#1a1a1a", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "8px", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            {["Név", "Slug", "Fordítások", "Műveletek"].map(h => <th key={h} style={{ padding: "0.875rem 1rem", textAlign: "left", color: "#666", fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.05em", fontFamily: "Geist Mono, monospace" }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {(items || []).map(item => {
